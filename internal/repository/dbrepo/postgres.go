...
func (m *postgresDBRepo) SearchAvailabilityByDatesByRoomID(start, end time.Time, roomdID int) (bool, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	...
	row := m.DB.QueryRowContext(ctx, query, roomdID, start, end)
...
// AllReservations returns a slice of all reservastions
...
// DeleteReservation Deteles one reservation by id
